...
import { Check, Download, FilePlus2, Focus, Loader2, Moon, PanelLeft, Search, Star, Sun, Trash2, Undo2, Redo2, Upload, X } from 'lucide-react'
    ...
    <div className="flex min-w-0 items-center gap-1 truncate text-xs text-zinc-400">
      {chain.map((page, index) => (
        <span key={page.id} className="flex min-w-0 items-center gap-1">
          <button onClick={() => onSelect(page.id)} className="max-w-[180px] truncate transition-colors hover:text-zinc-900 dark:hover:text-white">
...
export default function Workspace({ initialPageId }: { initialPageId?: string } = {}) {
  ...
  return (
    <div className={`flex min-h-screen bg-white text-zinc-900 dark:bg-zinc-950 dark:text-zinc-100 ${focusMode ? 'focus-mode' : ''}`}>
      ...
      {!focusMode && sidebarOpen && <>
        <aside className="hidden w-72 shrink-0 flex-col border-r border-zinc-200 bg-zinc-50/80 md:flex dark:border-zinc-800 dark:bg-zinc-900/50">
          <div className="flex items-center gap-2 px-3 py-3 text-sm font-semibold"><div className="grid size-7 place-items-center rounded-lg bg-zinc-900 text-white shadow-sm dark:bg-white dark:text-black">N</div><span>Nupoo</span><span className="ml-auto text-[10px] font-normal tracking-wide text-zinc-400">LOCAL</span></div>
          <div className="space-y-1 px-2">
            <button className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800" onClick={() => handleCreate(null)}><FilePlus2 size={16} /> Nová stránka <kbd className="ml-auto rounded border border-zinc-300 px-1 text-[10px] text-zinc-400">⌘N</kbd></button>
            <button className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800" onClick={() => setSearchOpen(true)}><Search size={16} /> Hledat <kbd className="ml-auto rounded border border-zinc-300 px-1 text-[10px] text-zinc-400">⌘K</kbd></button>
          </div>
          ...
          <div className="border-t border-zinc-200 p-2 dark:border-zinc-800">
            {recent.length > 0 && <div className="mb-2"><div className="px-2 py-1 text-[10px] font-semibold uppercase tracking-widest text-zinc-400">Nedávné</div>{recent.map((page) => <button key={page.id} onClick={() => navigate(page.id)} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800"><span aria-hidden="true">{page.icon || '📄'}</span><span className="min-w-0 flex-1 truncate text-left">{page.title || 'Bez názvu'}</span></button>)}</div>}
            <button onClick={() => setTrashOpen(true)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800"><Trash2 size={15} /> Koš {trash.length > 0 && <span className="ml-auto text-xs text-zinc-400">{trash.length}</span>}</button>
            <button onClick={() => downloadExport(pages, trash)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800"><Download size={15} /> Exportovat</button>
            <button onClick={() => importRef.current?.click()} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm transition-colors hover:bg-zinc-200 dark:hover:bg-zinc-800"><Upload size={15} /> Importovat</button>
