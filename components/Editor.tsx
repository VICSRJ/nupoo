...
export default function Editor({ page, onChange }: { page: Page; onChange: (page: Page) => void }) {
  ...
  const duplicateBlock = (index: number) => { const source = blocks[index]; if (!source) return; const duplicate = { ...source, id: nanoid(), content: source.content ? structuredClone(source.content) : undefined }; persist([...blocks.slice(0, index + 1), duplicate, ...blocks.slice(index + 1)]); setActiveBlockId(duplicate.id); setContext(null) }
