...
function TypeMenu({ block, onChange }: { block: Block; onChange: (type: BlockType, level?: 1 | 2 | 3) => void }) {
  return <div className="block-type-menu" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {BLOCK_TYPES.map((item) => <button key={item.type} type="button" onClick={() => onChange(item.type, item.type === 'heading' ? block.level || 1 : undefined)} className={`block-type-item ${block.type === item.type ? 'is-active' : ''}`}>
      <span className="block-menu-icon">{item.icon}</span><span>{item.label}</span><kbd className="ml-auto text-[9px] text-zinc-400">/{item.shortcut}</kbd>{block.type === item.type && <Check size={12} />}
    </button>)}
...
function SlashMenu({ query, onPick }: { query: string; onPick: (item: MenuItem) => void }) {
  ...
  if (!items.length) return <div className="slash-menu"><div className="px-3 py-3 text-xs text-zinc-400">Žádný příkaz.</div></div>
  return <div className="slash-menu" onMouseDown={(event) => event.preventDefault()}>{items.map((item) => <button key={item.type} type="button" onClick={() => onPick(item)} className="block-type-item"><span className="block-menu-icon">{item.icon}</span><span>{item.label}</span><kbd className="ml-auto text-[9px] text-zinc-400">/{item.shortcut}</kbd></button>)}</div>
}
...
function Row({ block, active, onActivate, onContextMenu, onUpdate, onDelete, onDuplicate, onAdd }: { block: Block; active: boolean; onActivate: () => void; onContextMenu: (event: React.MouseEvent) => void; onUpdate: (value: Partial<Block>) => void; onDelete: () => void; onDuplicate: () => void; onAdd: () => void }) {
    ...
    editorProps: {
      ...
      handleKeyDown: (_view, event) => {
        ...
        if (slashOpen && event.key === 'Backspace') {
          ...
          if (text.length <= 1) setSlashOpen(false)
          else setSlashQuery(text.slice(0, -1))
        }
        return false
      },
    },
  ...
  const changeType = (type: BlockType, level?: 1 | 2 | 3) => {
    ...
    onUpdate({ ...next, content: nextContent })
  ...
  const slashOffset = useMemo(() => (slashOpen && editor ? Math.min(editor.getText().length * 1, 120) : 0), [slashOpen, editor])
  ...
  return <div ref={setNodeRef} onFocusCapture={onActivate} onContextMenu={onContextMenu} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.55 : 1 }} className={`block-row relative flex min-w-0 items-start rounded-md ${active ? 'is-active' : ''}`}>
    ...
    <div className="relative min-w-0 flex-1 py-0.5">
      ...
      {slashOpen && editor && <div className="absolute left-0 top-full z-40" style={{ transform: `translateY(${Math.min(slashOffset, 10)}px)` }}><SlashMenu query={slashQuery} onPick={pickSlash} /></div>}
