...
type MenuItem = { type: BlockType; label: string; icon: React.ReactNode }

const BLOCK_TYPES: MenuItem[] = [
  { type: 'paragraph', label: 'Text', icon: <span className="text-xs font-semibold">T</span> },
  { type: 'heading', label: 'Nadpis', icon: <Heading size={14} /> },
  { type: 'bulletList', label: 'Odrážky', icon: <List size={14} /> },
  { type: 'orderedList', label: 'Číslování', icon: <ListOrdered size={14} /> },
  { type: 'taskList', label: 'Úkoly', icon: <ListCheck size={14} /> },
  { type: 'blockquote', label: 'Citace', icon: <Quote size={14} /> },
  { type: 'codeBlock', label: 'Kód', icon: <Code2 size={14} /> },
]
...
function TypeMenu({ block, onChange }: { block: Block; onChange: (type: BlockType, level?: 1 | 2 | 3) => void }) {
  return <div className="block-type-menu" onMouseDown={(event) => event.stopPropagation()}>
    ...
    {BLOCK_TYPES.map((item) => <button key={item.type} type="button" onClick={() => onChange(item.type, item.type === 'heading' ? block.level || 1 : undefined)} className={`block-type-item ${block.type === item.type ? 'is-active' : ''}`}>
      <span className="block-menu-icon">{item.icon}</span><span>{item.label}</span>{block.type === item.type && <Check size={12} />}
    </button>)}
    {block.type === 'heading' && <><div className="block-menu-separator" /><div className="block-menu-title">Úroveň</div>
      {[1, 2, 3].map((level) => <button key={level} type="button" onClick={() => onChange('heading', level as 1 | 2 | 3)} className={`block-type-item ${block.level === level ? 'is-active' : ''}`}>
        <span className="block-menu-icon heading-level">H{level}</span><span>Nadpis {level}</span>{block.level === level && <Check size={12} />}
      </button>)}
    </>}
  </div>
}
...
function InlineToolbar({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  return <div className="inline-toolbar" onMouseDown={(event) => event.preventDefault()}>
    <button className={`inline-tool ${editor.isActive('bold') ? 'is-active' : ''}`} onClick={() => editor.chain().focus().toggleBold().run()} aria-label="Tučné"><Bold size={14} /></button>
...
function Row({ block, active, onActivate, onContextMenu, onUpdate, onDelete, onDuplicate, onAdd }: { block: Block; active: boolean; onActivate: () => void; onContextMenu: (event: React.MouseEvent) => void; onUpdate: (value: Partial<Block>) => void; onDelete: () => void; onDuplicate: () => void; onAdd: () => void }) {
  ...
  const editor = useEditor({
    ...
    editorProps: { attributes: { 'aria-label': typeLabel(block) } },
    onFocus: onActivate,
    onUpdate: ({ editor: currentEditor }) => onUpdate({ text: currentEditor.getText(), content: currentEditor.getJSON() }),
  })

  useEffect(() => {
    if (!editor || editor.isFocused) return
    const incoming = blockContent(block)
    if (JSON.stringify(editor.getJSON()) !== JSON.stringify(incoming)) editor.commands.setContent(incoming, false)
  }, [block, editor])

  const changeType = (type: BlockType, level?: 1 | 2 | 3) => {
    const next: Block = { ...block, type, level: type === 'heading' ? level || 1 : undefined }
    onUpdate(next)
    editor?.commands.setContent(blockContent({ ...next, content: undefined }), false)
  ...
  return <div ref={(node) => { rowRef.current = node; setNodeRef(node) }} onFocusCapture={onActivate} onContextMenu={onContextMenu} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.55 : 1 }} className={`block-row relative flex min-w-0 items-start rounded-md ${active ? 'is-active' : ''}`}>
    <div className={`block-toolbar ${active ? 'is-visible' : ''}`} onMouseDown={(event) => event.preventDefault()}>
      <button type="button" onClick={onAdd} aria-label="Přidat blok" title="Přidat blok" className="block-control"><Plus size={13} /></button>
      <div className="relative">
        <button type="button" onClick={() => setTypeMenuOpen((value) => !value)} aria-label={`Změnit typ: ${typeLabel(block)}`} title={`Změnit typ: ${typeLabel(block)}`} className={`block-control ${typeMenuOpen ? 'is-pressed' : ''}`}><span className="flex items-center">{BLOCK_TYPES.find((item) => item.type === block.type)?.icon}</span></button>
        {typeMenuOpen && <TypeMenu block={block} onChange={changeType} />}
      </div>
      <button type="button" {...listeners} {...attributes} aria-label="Přesunout blok" title="Přesunout blok" className="block-control cursor-grab active:cursor-grabbing"><GripVertical size={13} /></button>
    </div>
    <div className="relative min-w-0 flex-1 py-0.5">
      {active && editor && <InlineToolbar editor={editor} />}
...
export default function Editor({ page, onChange }: { page: Page; onChange: (page: Page) => void }) {
  ...
  const addAt = (index: number) => {
    const nextBlock: Block = { id: nanoid(), type: 'paragraph', text: '', content: { type: 'doc', content: [{ type: 'paragraph' }] } }
