...
type NupooStore = { pages: Page[]; trash: Page[]; activePageId: string; sidebarOpen: boolean; dark: boolean; searchOpen: boolean; query: string; saveState: SaveState; hydrated: boolean; canUndo: boolean; canRedo: boolean; initialize: (requestedId?: string) => void; selectPage: (id: string) => void; createPage: (parentId?: string | null) => string; updatePage: (page: Page) => void; deletePage: (id: string) => void; restorePage: (id: string) => void; permanentlyDeletePage: (id: string) => void; emptyTrash: () => void; undo: () => void; redo: () => void; toggleFavorite: (id: string) => void; setSidebarOpen: (open: boolean) => void; toggleDark: () => void; setSearchOpen: (open: boolean) => void; setQuery: (query: string) => void; setSaveState: (state: SaveState) => void }
...
export const useNupooStore = create<NupooStore>((set, get) => ({
  ...
  initialize: (requestedId) => { const pages = dataService.load(); const trash = dataService.loadTrash(); const activePageId = requestedId && pages.some((page) => page.id === requestedId) ? requestedId : pages[0]?.id || ''; historyPast = []; historyFuture = []; lastHistoryAt = 0; lastHistoryPageId = ''; set({ pages, trash, activePageId, hydrated: true, canUndo: false, canRedo: false }) },
  ...
  deletePage: (id) => set((state) => { if (id === 'welcome') return state; const ids = new Set<string>(); const walk = (parentId: string) => { ids.add(parentId); state.pages.filter((page) => page.parentId === parentId).forEach((page) => walk(page.id)) }; walk(id); const removed = state.pages.filter((page) => ids.has(page.id)); if (!removed.length) return state; historyPast = [...historyPast, snapshot(state.pages)].slice(-80); historyFuture = []; const timestamp = new Date().toISOString(); const pages = state.pages.filter((page) => !ids.has(page.id)); const trash = [...state.trash, ...removed.map((page) => ({ ...page, parentId: null, trashedAt: timestamp }))]; const activePageId = ids.has(state.activePageId) ? pages[0]?.id || '' : state.activePageId; return { pages, trash, activePageId, canUndo: true, canRedo: false } }),
  restorePage: (id) => set((state) => { const item = state.trash.find((page) => page.id === id); if (!item) return state; historyPast = [...historyPast, snapshot(state.pages)].slice(-80); historyFuture = []; return { trash: state.trash.filter((page) => page.id !== id), pages: [...state.pages, { ...item, trashedAt: null }], activePageId: item.id, canUndo: true, canRedo: false } }),
